...
def TestOverStep():
    ...
    for i in xrange(5):
        ...
        for i in xrange(numOfTrial):
            p1 = AOPSO.PSOProblem(nDim, numOfParticles, maxIteration, minX, maxX, minV, maxV, AOPSO.sphere,intDim,step)
            p1.run()
            gBest = gBest + p1.gBestArray[:maxIteration]
        gBest = gBest / numOfTrial
        pylab.plot(range(maxIteration), gBest,label='step'+str(step))
    pylab.title('Sphere function, intDim=5 ($G_{best}$ over 20 trials)')
